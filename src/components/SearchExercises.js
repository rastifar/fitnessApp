...
import Imk from "../assets/images/pic.svg";
  ...
  const handleSearch = async () => {
    if (search) {
      ...
      const searchedExercise = exercisesData.filter(
        (exercise) =>
          exercise.name.toLowerCase().inclue(search) ||
          exercise.target.toLowerCase().inclue(search) ||
          exercise.equipment.toLowerCase().inclue(search) ||
          exercise.bodyPart.toLowerCase().inclue(search)
      );
      setSearch("");
